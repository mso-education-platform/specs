...
* For admin role: dedicated notifications view for pending parent follow requests
...
* Parent follow-access request workflow submitted to admins for validation
...
* **Given** a parent is not yet linked to a learner, **When** requesting follow access, **Then** an admin receives a pending request notification.
* **Given** an admin who can also act as educator, **When** reviewing parent requests, **Then** they can approve or reject each request.
* **Given** pending parent follow requests exist, **When** admin opens notifications view, **Then** they can see unread items and mark notifications as read.
...
* **FR-013**: System MUST allow parents to submit follow-access requests for a learner and notify admins of pending validations
* **FR-014**: System MUST allow admin reviewers (including educator-admin accounts) to approve or reject parent follow requests
...
* **ParentFollowRequest**: Parent-submitted request to follow a learner, with pending/approved/rejected lifecycle
* **AdminNotification**: Notification item for admins when a parent follow request requires review
